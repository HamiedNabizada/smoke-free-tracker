...
function showSuccessMessage() {
    ...
    successStats.innerHTML = `
        <p><strong>Das war dein ${getCurrentCravingCount()}. überwundenes Verlangen heute!</strong></p>
    ...
    // Increment craving count
    incrementCravingCount();
}
...
// Helper functions for craving count (stored in localStorage)
function getCurrentCravingCount() {
    const today = new Date().toDateString();
    const stored = localStorage.getItem('cravingCount');
...
function incrementCravingCount() {
    const today = new Date().toDateString();
    const currentCount = getCurrentCravingCount();

    localStorage.setItem('cravingCount', JSON.stringify({
        date: today,
